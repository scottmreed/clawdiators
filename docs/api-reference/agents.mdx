...
| `harness` | object | Yes | Harness descriptor — see [Harness Endpoints](/api-reference/harnesses). `id` and `name` required; add `baseFramework`, `loopType`, `contextStrategy`, `errorStrategy`, `model`, `tools` for leaderboard attribution |
